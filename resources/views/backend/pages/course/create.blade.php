...
@section('b-content')
    <section class="course-section">
        ...
        @endif
        <form action="{{ route('courses.store') }}" method="POST" class="course-form" enctype="multipart/form-data">
            @csrf
            <h1 class="course-heading animate__animated animate__rubberBand">Upload Course</h1>
            <div class="course-field">
                <label for="" class="course-label">Title</label>
                <input type="text" name="title" id="" class="course-input" value="{{ old('title') }}">
                @error('title')
                    <span class="text text-danger">{{ $message }}</span>
                @enderror
            </div>
            <div class="course-field">
                <label for="" class="course-label">Picture</label>
            ...
            <div class="course-field">
                <label for="" class="course-label">Slug</label>
                <input type="text" name="slug" id="" class="course-input" value="{{ old('slug') }}">
                @error('slug')
                    <span class="text text-danger">{{ $message }}</span>
                @enderror
            </div>
            <div class="course-field">
                <label for="" class="course-label">Short Description</label>
                <input type="text" name="short_description" value="{{ old('short_description') }}" id="" class="course-input">
                @error('short_description')
                    <span class="text text-danger">{{ $message }}</span>
                @enderror
            </div>
            <div class="course-field">
                <label for="" class="course-label">Total Price</label>
                <input type="text" id="" name="price" value="{{ old('price') }}" class="course-input">
                @error('price')
                    <span class="text text-danger">{{ $message }}</span>
                @enderror
            </div>
